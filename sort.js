...
function setRandomBars() {
  ...
  async function shuffle(heights) {
    ...
    while (0 !== currentIndex) {
      ...
      bars[currentIndex].style.backgroundColor = "#2d748a";
      bars[randomIndex].style.height = heights[randomIndex] + "px";
      bars[randomIndex].style.backgroundColor = "#2d748a";
      await sleep(5);
    }
...
async function bubbleSort() {
  ...
  for (let i = len - 1; i >= 0; i--) {
    ...
    for (let j = 1; j <= i; j++) {
      bars[j].style.backgroundColor = "#8899ff";
     bars[j-1].style.backgroundColor = "#2d748a";
...
async function mergeSort() {
  ...
  for (curr_size = 1; curr_size <= n - 1; curr_size = 2 * curr_size) {
    ...
    for (left_start = 0; left_start < n - 1; left_start += 2 * curr_size) {
      ...
      k = l;
      while (i < n1 && j < n2) {
        bars[k].style.backgroundColor = "#8899ff";
        //bars[j-1].style.backgroundColor = "#2d748a";
        if (ahead == false) return;
        if (L[i] <= R[j]) {
          heights[k] = L[i];
    ...
    for (let j = 0; j < counter.length; j++) {
      ...
      if (counter[j] != undefined) {
        while ((value = counter[j].shift()) != undefined) {
          ...
          bars[pos - 1].style.backgroundColor = "#8899ff";
          await sleep(300);
        }
      }
    }
